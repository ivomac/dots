...
local function buftransform(buf)
  ...
  for _, timevar in ipairs(timevars) do
    ...
    local time = buf.variables["timesaver_" .. var] or 0
    if time == 0 then
      item[var] = "_"
    else
      if ref then
...
local MRW_opts = {
  ...
  sort = function(buf1, buf2)
    ...
    local w1 = buf1.variables.timesaver_twrite
    local w2 = buf2.variables.timesaver_twrite
...
local LFT_opts = {
  filter = function(buf)
    return
        buf.loaded == 1
        and (buf.variables.timesaver_tfocused or 0) > 0
  end,
  sort = function(buf1, buf2)
    return
        buf1.variables.timesaver_tfocused > buf2.variables.timesaver_tfocused
  end,
  transform = buftransform,
  prompt = " LFT ",
  format_item = {
...
return {
...

  {
    ...
    cmd = { "Explorer", "LazyGit" },
    opts = {},
    keys = {
      { mode = { "n" }, "<C-l>",       function() require("floatterm").last() end,          silent = true, noremap = true, desc = "Toggle last term" },
      ...
      { mode = { "n" }, "<leader>te",  function() require("floatterm").explorer() end,      silent = true, noremap = true, desc = "Explorer" },
