-- local codeium_ns = vim.api.nvim_create_namespace("codeium_indicator")
-- local codeium_id = 999
...
return {
		...
		opts = {
			...
			sections = {
				...
				lualine_b = {
					...
					-- 			vim.api.nvim_buf_set_extmark(0, codeium_ns, pos[1] - 1, pos[2], {
					-- 				id = codeium_id,
					-- 				virt_text = { { " 󰚩 …", "CodeiumSuggestion" } },
					-- 				virt_text_pos = "overlay",
					-- 			})
					-- 			return "󰚩 …"
					-- 		end
					--
					-- 		vim.api.nvim_buf_del_extmark(0, codeium_ns, codeium_id)
					--
				...
				lualine_y = {
					...
					{ "filetype", icon_only = true, colored = false, padding = { left = 0, right = 1 }, },
...

	{
		...
		event = "VeryLazy",
		config = function()
			local statcol = require("statuscol")
			statcol.setup({
				segments = {
					{
						sign = { text = { ".*" }, maxwidth = 1, colwidth = 1, auto = false, wrap = false },
					},
				}
			})
		end,
