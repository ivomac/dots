...
local function save_session(session)
  ...
  for _, buf in ipairs(vim.fn.getbufinfo({ buflisted = 1 })) do
    for varname, varval in pairs(buf.variables) do
      ...
      if vim.tbl_contains(M.opts.savevartype, vartype) and not vim.tbl_contains(M.opts.skipvar, varname) then
  ...
  M.opts = vim.tbl_deep_extend("force",
    {
      ...
      autosave = true,
      savevartype = {},
      skipvar = { "did_ftplugin", "undo_ftplugin", "did_indent", "undo_indent" },
    },
    opts or {}
  )
